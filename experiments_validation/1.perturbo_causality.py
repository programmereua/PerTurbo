...
SECTIONS = {
    "HM11": ("handoff_to_eva/data/IU_PDA_HM11.h5ad",
             "handoff_to_eva/drivers/HM11_drivers_per_CC.csv", "liver metastasis"),
    "T11":  ("handoff_to_eva/data/IU_PDA_T11.h5ad",
             "handoff_to_eva/drivers/T11_drivers_per_CC.csv",  "primary tumour"),
}

# The driver tables are per Cell Community (columns: CC, theme, role, gene, logFC, padj, score),
# NOT per RCTD cell type. So the unit of analysis is the Cell Community, and the matching spot
# label in the h5ad is `CompositionCluster_CC` (values like CC4, CC8, CC12 ...), which lines up
# exactly with the `CC` column of the driver CSV.
#
# The previous config grouped spots by `first_type` and looked for a `cell_type` column in the
# driver table. Neither matched this data: `cell_type` does not exist in the CSV, so every
# population silently produced an empty driver list and was skipped -- the script would finish
# with zero results and no error. Grouping by CC fixes that.
POPULATION_COL = "CompositionCluster_CC"   # spot -> Cell Community, in a.obs
DRIVER_POP_COL = "CC"                       # the matching column in the driver CSV
THEME_COL, SAMPLE_ID = "theme", "sample_id"
...
def run_section(name, h5, drvp, label):
    ...
    ct = a.obs[POPULATION_COL].astype(str).values   # each spot's Cell Community (CC4, CC8, ...)
    ...
    for pop, n_sp in pop_counts.items():
        # select this Cell Community's drivers. Restrict to perturbation targets where that
        # role column exists, so we do not knock out genes the table only used for training.
        if DRIVER_POP_COL not in drv.columns:
            raise KeyError(
                f"driver table has no '{DRIVER_POP_COL}' column (found: {list(drv.columns)}). "
                f"Set DRIVER_POP_COL to whichever column holds the population label.")
        sel = drv[drv[DRIVER_POP_COL].astype(str) == str(pop)]
        if "role" in sel.columns and (sel.role == "perturb_target").any():
            sel = sel[sel.role == "perturb_target"]
        sub = sel.head(TOP)
